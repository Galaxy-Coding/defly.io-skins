  ...
  main.onreadystatechange = function() {
	...
	if (this.readyState == 0) {
	  ...
	  for(i=0;i<data.length;i++){
		  arrayz.push(`<div class="bg-dark card"><div class="card-body m-6 shadow-sm text-center"><h3>${data[i]}</h3> <div class=btn-group><a class="btn btn-light" href="${data[i]}">Download</a> <button data-clipboard-text="${data[i]}" class="btn btn-light">Copy Link</button></div></div></div>`);
	  }
